...
- **Common color themes.** Add well-known palettes people recognize — Dracula, Catppuccin (Mocha/Latte), maybe Nord / Solarized / Gruvbox. Each is just another `:root[data-theme="…"]` variable block plus a `THEMES` entry, same as the existing themes.
